...
func checkFile(filePath string) {
	...
	// We defer closing the file to ensure it doesn't leak.
	defer file.Close()
	...
	var mu sync.Mutex // Mutex added to prevent concurrent map write panics
	changed := false
	...
	for pn, p := range providers {
		wg.Add(1)
		// Pass pn and p.Url into the goroutine to prevent loop variable capture issues (if pre-Go 1.22)
		go func(providerName, providerUrl string) {
			defer wg.Done()

			reqCtx, cancel := context.WithTimeout(ctx, time.Second*5)
			defer cancel()

			URL, err := checkUrl(reqCtx, providerUrl)
			if err != nil {
				errCh <- err
				return
			}

			mu.Lock() // Lock before writing to the shared map and boolean
			if !changed && URL != providerUrl {
				changed = true
			}
			providers[providerName].Url = URL
			mu.Unlock()
	...
	if changed {
		...
		// Replaced NewEncoder with MarshalIndent
		indentedJSON, err := json.MarshalIndent(providers, "", "  ")
		if err != nil {
			fmt.Fprintf(os.Stderr, "error marshaling json: %v\n", err)
		} else {
			if _, err := file.Write(indentedJSON); err != nil {
